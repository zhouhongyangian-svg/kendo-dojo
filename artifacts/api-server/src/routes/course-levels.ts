...
router.post("/", requireCourseAdmin, async (req: Request, res: Response) => {
  try {
    ...
    if (err?.code === "23505") return res.status(400).json({ error: "该种类名称已存在" });
    return res.status(500).json({ error: "Internal server error" });
  }
});
...
router.patch("/:id", requireCourseAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const { name, color, sortOrder } = req.body;
    const oldLevel = await db.select().from(courseLevelsTable).where(eq(courseLevelsTable.id, id)).limit(1);
    ...
    const [updated] = await db.update(courseLevelsTable).set(updates).where(eq(courseLevelsTable.id, id)).returning();
...
router.delete("/:id", requireCourseAdmin, async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id);
    const [deleted] = await db.delete(courseLevelsTable).where(eq(courseLevelsTable.id, id)).returning();
