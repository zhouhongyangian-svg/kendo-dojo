...
import { requireAuth, requireAdmin, requireCourseAdmin } from "../middlewares/requireAuth";
...
router.get("/upcoming", requireAuth, async (_req: Request, res: Response) => {
  try {
    ...
    const courses = await db
      .select()
      .from(coursesTable)
      .where(and(gte(coursesTable.scheduledAt, now), lte(coursesTable.scheduledAt, thirtyDaysLater)))
    ...
    const withCounts = await Promise.all(
      courses.map(async (course) => {
        const [countResult] = await db
          .select({ count: count() })
          .from(bookingsTable)
          .where(and(eq(bookingsTable.courseId, course.id), eq(bookingsTable.status, "confirmed")));
        return {
          ...course,
          scheduledAt: course.scheduledAt.toISOString(),
          enrolledCount: Number(countResult?.count ?? 0),
...
router.get("/", requireAuth, async (_req: Request, res: Response) => {
  try {
    const courses = await db.select().from(coursesTable).orderBy(coursesTable.scheduledAt);
...
router.post("/", requireCourseAdmin, async (req: Request, res: Response) => {
  try {
    const { title, description, instructor, scheduledAt, durationMinutes, maxCapacity, level } = req.body;
    if (!title || !instructor || !scheduledAt || !durationMinutes || !maxCapacity) {
...
router.patch("/:id", requireCourseAdmin, async (req: Request, res: Response) => {
  try {
...
router.delete("/:id", requireCourseAdmin, async (req: Request, res: Response) => {
  try {
    const id = parseInt(req.params.id);
